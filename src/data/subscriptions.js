...
import { getCurrentTimeInTimezone, getTimezoneMidnightTimestamp, addGregorianPeriod } from '../core/time.js';
  ...
  try {
    ...
    if (useLunar) {
      ...
      if (expiryDate < currentTime && subscription.periodValue && subscription.periodUnit) {
        // 优先从 startDate 提取 anchorDay，避免使用已被前端钳位的 expiryDate
        const anchorDay = subscription.startDate
...
async function updateSubscription(id, subscription, env) {
  try {
    ...
    if (useLunar) {
      ...
      if (expiryDate < currentTime && subscription.periodValue && subscription.periodUnit) {
        // 优先从已有 anchorDay 或 startDate 提取，避免使用已钳位的 expiryDate
        const anchorDay = subscriptions[index].anchorDay
          || (subscription.startDate ? new Date(subscription.startDate).getDate() : expiryDate.getDate());
        while (expiryDate < currentTime) {
          expiryDate = addGregorianPeriod(expiryDate, subscription.periodValue, subscription.periodUnit, anchorDay);
        }
        subscription.expiryDate = expiryDate.toISOString();
...
async function manualRenewSubscription(id, env, options = {}) {
  try {
    ...
    subscriptions[index] = {
      ...subscription,
      anchorDay: subscription.anchorDay || anchorDay,
