...
import { getCurrentTimeInTimezone, MS_PER_HOUR, MS_PER_DAY, getTimezoneMidnightTimestamp, addGregorianPeriod } from '../core/time.js';
    ...
    for (const subscription of subscriptions) {
      ...
      if (subscription.autoRenew && daysDiff < 0) {
        ...
        if (subscription.useLunar) {
          ...
          const anchorDay = subscription.anchorDay || expiryDate.getDate();
          while (expiryDate <= currentTime) {
            if (mode === 'reset') {
